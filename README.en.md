...
1. Download the code: grab the latest ZIP from [Releases](https://github.com/JingW-ui/OpenJobAutofill/releases/latest) and unzip it, or clone this repository.
2. Open `brave://extensions/` or `chrome://extensions/`.
3. Turn on `Developer mode`.
4. Click `Load unpacked`.
5. Select the unzipped/cloned `OpenJobAutofill` folder.
...
2. Click `Settings`.
3. Fill in your resume profile by section — or use "Import from resume (AI)" to parse pasted text / files, or "Parse this page as resume" from the popup while on your online resume page.
4. Click `Save Profile`; the data is saved to local browser extension storage.
5. (Optional) For different positions: duplicate the current version in the Settings version bar and tailor it; switch the active version in the popup before filling.
6. Open a resume or application form page on a recruiting website.
7. In the popup, check the "Only fill blank fields" toggle (on by default — existing page content is never overwritten), then click `Start Filling`.
8. Wait for scanning and filling to complete, then review the green/orange marks on the page. After manually filling an orange field, click "Remember" in the result panel to reuse it forever.
...
This fork ships with a working intranet proxy default (`127.0.0.1:15721`) — zero configuration needed, and legacy configs auto-migrate on upgrade. Click "Restore intranet default" in Settings if things get messed up. You can also switch entirely to your own OpenAI-compatible endpoint or custom request template.

AI works in three places with different privacy boundaries:

- **Page-field recognition (during autofill)**: AI only receives page field names and your profile field catalog — never your name, phone, or experience content. Values resolve and fill locally.
- **Resume import / parse current page**: these two features send the resume text (or page text) to your configured AI endpoint once for structuring, with an explicit notice before triggering.
- **Separate parse model**: the "Parse model" defaults to `auto_deepseek_plan` (fast, ~20s); leave it empty to follow the main model. If you switch to your own API, leave it empty or use a model that exists there. Parsing streams with live progress in the corner of the page.

Without any API configured, autofill still works (local rules + Q&A memory fast path), but resume parsing features are unavailable.
...
- During autofill the extension never sends your actual resume values to AI (field names only); only "resume import / parse current page" send the resume text to your configured AI endpoint, with an explicit notice beforehand.
...
### Resume parsing is slow or stuck

A typical parse takes about 20 seconds (streaming, with live progress at the page corner). If it stalls for over 90 seconds, the state auto-flips to failed — click "Retry" in the popup to reuse the extracted text without re-reading the page. If failures persist, change the "Parse model" in Settings or check your API connection.

### How do I maintain different resumes for different positions?

Use the version bar on top of the profile editor (New / Duplicate / Rename / Delete). Recommended: keep the main resume complete, then "Duplicate" it per position direction and only tailor the intention/self-description. Switch the active version in the popup before filling. Remembered Q&A answers live in the main resume and apply to all versions.
