# OpenJobAutofill

[English README](README.en.md)

<p align="center">
  <img src="assets/openjobautofill-logo.png" alt="OpenJobAutofill" width="720" />
</p>

OpenJobAutofill 是一个带 AI 页面分析能力的网申表单填写扩展。你只需要在本机维护一份简历资料，之后打开招聘网站，点击 `开始填写`，它会扫描当前页面，用本地规则和可选 AI 理解字段含义，把能确定的内容填进表单，其他字段标为待处理。

它的重点不是“全自动投递”，而是把 AI 用在最适合的地方：理解不同网站五花八门的字段名称和页面结构，同时把简历具体内容留在本机。个人信息、教育经历、实习经历、项目经历、证书、奖项这些反复填写的内容，都可以交给它先填一遍。最后仍由你自己检查并提交。

如果这个项目帮你节省了网申时间，欢迎给仓库点一个 Star。开源不易，你的反馈也会帮助我继续优化更多招聘网站的兼容性。

## 功能亮点

- 简历版本管理：一份主简历 + 多份岗位子简历，点击插件图标即可在弹窗切换当前填写版本；问答记忆统一保存在主简历，对所有版本生效。
- 一键扫描当前招聘页面，自动填写能确定的字段，其他字段标为待处理。
- 只填空白项开关（默认开启）：页面已有内容的字段一律跳过不覆盖，统计里会显示跳过数量；关闭后恢复"高置信度才覆盖"策略。
- 支持从简历文本或 .txt / .md / .html / .pdf 文件 AI 解析导入：解析结果先合并进资料编辑器供人工复核，确认后再保存（注意：此功能会把简历原文发送到你配置的 AI 接口；PDF 在本机用内置 pdf.js 抽取文字，扫描件图片型 PDF 请改用文本粘贴）。
- 支持解析当前页面为简历：在在线简历页（如 BOSS/智联的"我的简历"）点击插件弹窗里的「解析当前页为简历」，自动抽取页面文本并 AI 解析为新版本草稿，打开设置页复核保存后即可使用。解析在后台进行，弹窗关闭不影响；页面右下角有实时进度；不像简历页会先确认；失败可一键重试（复用已抽取文本）。
- 简历资料只保存在本机，不需要把个人信息上传到云端。
- 支持常见输入框、文本域、单选、多选、下拉框和日期类字段。
- 资料面板可以按分类查看、搜索和手动复制内容，帮助处理橙色待处理字段。
- 可选接入 OpenAI 兼容 API 或自定义 API，帮助理解不同网站的字段含义。
- AI 只辅助识别页面字段和可匹配的资料项，不接收你的简历具体值。
- 页面填写结果会用两种颜色标记：绿色已填写，橙色待处理。
- 问答记忆：橙色待处理字段在页面上手动填好后，可在填写结果面板一键"记住"。答案只保存在本机，并自动加入资料目录（设置页的"问答记忆"模块可查看、编辑、删除）；下次开始填写时优先本地精确匹配复用，AI 语义匹配兜底。
- 支持检查 GitHub Release 更新；有新版本时浏览器图标会显示 `NEW`。

## 安装

当前版本适合以开发者模式安装到 Brave 或 Chrome。

1. 下载代码：推荐到 [Releases](https://github.com/JingW-ui/OpenJobAutofill/releases/latest) 下载最新版 ZIP 并解压；也可以克隆本项目。
2. 打开 `brave://extensions/` 或 `chrome://extensions/`。
3. 开启右上角 `Developer mode`。
4. 点击 `Load unpacked`。
5. 选择解压/克隆出的 `OpenJobAutofill` 目录。
6. 固定扩展图标，方便在招聘页面使用。

本项目不需要安装依赖，也不需要构建，加载项目目录即可使用。

## 第一次使用

1. 点击浏览器右上角的 OpenJobAutofill 图标。
2. 点击 `设置`。
3. 在 `简历资料` 中按模块填写你的信息；也可以展开「从简历导入（AI 解析）」，粘贴简历全文或选择简历文件，让 AI 先帮你填一遍再人工复核；如果当前就在在线简历页，也可以直接点弹窗里的「解析当前页为简历」。
4. 点击 `保存资料`，资料会保存到本机浏览器扩展存储。
5. （可选）为不同岗位建子简历：设置页版本栏点「复制当前版本」，改出岗位定制版；填写前在插件弹窗切换「当前简历版本」。
6. 打开招聘网站的简历填写页或申请表单页。
7. 点击扩展图标，确认「只填空白项」开关符合预期（默认开启，不覆盖页面已有内容），再点击 `开始填写`。
8. 等待扫描和填写完成，根据页面上的绿色/橙色标记检查结果；橙色字段在页面上手动填好后，可在结果面板点「记住」永久复用。
9. 最后由你自己确认页面内容并手动提交。

项目里提供了 `sample-profile.json`，如果你只是想先试一下效果，可以在设置页导入这份示例资料。

## AI 设置

本 fork 默认内置内网代理（`127.0.0.1:15721`），开箱即用，无需任何配置；老版本升级上来的旧配置会自动迁移。如果误改乱了，设置页点「恢复内网默认」一键还原。也可以完全改为你自己的 OpenAI 兼容接口或自定义请求模板。

AI 在三个地方工作，隐私边界不同，请注意：

- **页面字段识别（自动填写时）**：AI 只收到页面字段名和资料字段目录，**不包含**你的姓名、手机号、经历内容等具体资料值；真正的取值和填写都在本机完成。
- **简历解析导入 / 解析当前页**：这两个功能会把简历原文（或页面文本）发送到你配置的 AI 接口做一次性结构化解析，触发前界面有明确提示。
- **解析模型可单独配置**：「解析模型」默认 `auto_deepseek_plan`（快，约 20 秒），留空则跟随主模型；换成你自己的 API 时建议留空或填该平台存在的模型名。解析全程走流式传输，页面右下角能看到实时进度。

不配置任何 API 时，自动填写仍可工作（本地规则 + 问答记忆快车道），但简历解析类功能不可用。

## 更新

插件会定期检查 GitHub Release，也可以在弹窗或设置页手动点击 `检查更新`。如果发现新版本，浏览器图标会显示 `NEW`，点击 `打开 Release 页面` 后按发布页说明下载并覆盖或重新加载扩展。

更新前建议先在设置页点击 `导出资料备份`。更新时不要先卸载扩展；只要保留同一个浏览器扩展，本机保存的简历资料和 API 设置会继续保留。

## 颜色标记

- 绿色：已填写。
- 橙色：待处理，需要你手动处理或复核。

如果页面刷新、进入下一步或动态加载出新的表单，可以再次点击 `开始填写`。

## 隐私说明

- 简历资料保存在本机浏览器里。
- API Key 也只保存在本机扩展存储里。
- 页面脚本只会在你点击扩展并操作当前页面后注入。
- 插件不会自动点击最终提交按钮。
- 自动填写时插件不会把你的简历具体内容发送给 AI（仅字段名）；仅「简历解析导入 / 解析当前页」两个功能会把简历原文发给你配置的 AI 接口，且触发前有明确提示。
- 检查更新只访问本项目的 GitHub Release，不会上传简历资料。
- 页面自动填写后仍建议人工复核，尤其是证件、联系方式、日期、选择题和声明类字段。

## 常见问题

### 点击开始填写没有反应

先刷新目标页面，再重新打开扩展弹窗。如果仍无反应，可以到扩展管理页重载 OpenJobAutofill。

### 为什么有些下拉框或日期填不进去

不同招聘系统的控件实现差异很大。有些控件不是普通输入框，而是复杂组件。遇到这种情况，字段会进入待处理状态，可以用资料面板搜索对应内容并手动选择；如果某类网站经常需要手动处理，欢迎提 Issue，我会优先看可复用的适配方式。

### 多页表单怎么填

每进入一个新页面或新步骤后，再点击一次 `开始填写`。OpenJobAutofill 不会自动跨页面继续填写，也不会自动提交申请。

### 解析简历很慢或卡住怎么办

正常情况下解析一份简历约 20 秒（流式，页面右下角有实时进度）。如果卡住超过 90 秒，状态会自动转为失败，点弹窗里的「重试解析」即可复用已抽取的文本重试，不会重复读取页面。如果经常失败，可以在设置页把「解析模型」改为其他可用模型，或检查 API 连接。

### 怎么为不同岗位维护不同简历

设置页简历资料顶部的版本栏支持「新建 / 复制 / 重命名 / 删除」。推荐做法：主简历维护全量信息，点「复制当前版本」为每个岗位方向建子简历，只改求职意向、自我描述等定制部分。填写前在插件弹窗切换版本。「记住」的问答答案统一保存在主简历，对所有版本生效。

### 如何备份或迁移资料

在设置页使用 `导出资料备份` 和 `导入资料备份`。导出的文件是 OpenJobAutofill 自己的备份格式，主要用于换浏览器或换电脑时迁移资料。

### 如何清空本机资料

进入设置页，点击 `清空资料和 API 设置`。这会删除当前浏览器里保存的简历资料和 API 配置。

## 反馈

如果你遇到问题、希望适配某个招聘网站，或者有功能建议，欢迎直接提 Issue。反馈时尽量附上网站名称、页面截图、待处理字段描述，这样更容易定位问题。

也可以到我的 GitHub 主页查看公开邮箱联系我。

友情链接：[LINUX DO](https://linux.do) - 一个面向技术爱好者的中文社区，本项目链接并认可 LINUX DO，欢迎佬友交流和反馈。

## License

OpenJobAutofill 使用 MIT License 开源。详见 [LICENSE](LICENSE)。
